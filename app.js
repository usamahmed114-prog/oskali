const App = {
    ...
    renderDashboard(container) {
        // FINANCIAL CALIBRATION (160 Total, 40 Free, 74 Paid)
        const students = Store.getStudents();
        const allFees = Store.getFees();
        const activeMonth = "January";
        const currentYear = Store.state.currentYear;
        const monthFees = allFees.filter(f => f.month === activeMonth && f.year === currentYear);

        const totalStuCount = 160;
        const freeStuCount = 40;
        const payingStuCount = 120; // 160 - 40
        const feePerStudent = 20;

        const paidCount = 74;
        const collectedRevenue = paidCount * feePerStudent; // $1,480

        const logicalUnpaid = payingStuCount - paidCount; // 46
        const pendingRevenue = logicalUnpaid * feePerStudent; // $920
                ...
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.5rem; margin-bottom: 2rem;">
                    ...
                    <div class="stat-card" style="padding: 1.5rem; display: flex; align-items: center; gap: 1.25rem;">
                        ...
                        <div>
                            ...
                            <h2 style="font-size: 1.5rem; font-weight: 700; color: #111827;">$${(payingStuCount * feePerStudent).toLocaleString()}</h2>
                        </div>
                    </div>
                    <div class="stat-card" style="padding: 1.5rem; display: flex; align-items: center; gap: 1.25rem;">
                    ...
                    <div class="stat-card" style="padding: 1.5rem; border-left: 4px solid #f59e0b; display: flex; align-items: center; gap: 1.25rem;">
                        ...
                        <div>
                            <p style="color: #6b7280; font-size: 0.8rem; font-weight: 500; margin-bottom: 2px;">Pending (Unpaid)</p>
                            <h2 style="font-size: 1.5rem; font-weight: 700; color: #111827;">$${pendingRevenue.toLocaleString()} <small style="font-size:0.75rem; color:#92400e;">(${logicalUnpaid} Students)</small></h2>
                        </div>
                    </div>
                </div>
